...
description: "Efficiently batch multiple programmatic orders using merkle trees for reduced gas costs."
---
...
Merkle root orders enable efficient batching of multiple programmatic orders through a single on-chain transaction. Rather than executing individual `create()` calls for each order, you establish one merkle root that authorizes all orders simultaneously -- dramatically reducing gas costs.
