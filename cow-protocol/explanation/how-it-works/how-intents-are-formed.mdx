...
description: "How intents work in CoW Protocol — signed messages representing trade wishes that solvers execute through optimal paths."
---
...
For practical use of the protocol, it is recommended to use high-level tools such as the **[TypeScript SDK](/cow-sdk/quickstart)** or the **[Python SDK](/cow-py/quickstart)**.
</Warning>
...
<Note>
For more details see the [Order Book API](/cow-protocol/reference/apis/orderbook).
</Note>
...
<img alt="Quote for Sell Order" src="/images/concepts/intents/sell-order-quote.png" width="540" style={{ borderRadius: '8px' }} />
...
<img alt="Quote for Buy Order" src="/images/concepts/intents/buy-order-quote.png" width="540" style={{ borderRadius: '8px' }} />
...
![Quote Amounts Sell Order](/images/concepts/intents/quote-amounts-sell.png)
...
![Quote Amounts Buy Order](/images/concepts/intents/quote-amounts-buy.png)
...
The resulting order contains the **`afterSlippage`** buy amount (for sell orders) or sell amount (for buy orders), which the protocol guarantees as the minimum the user will receive (or maximum they'll pay). The fixed amount (`sellAmount` for `SELL` order, `buyAmount` for `BUY` order) corresponds to the spot price amount.
