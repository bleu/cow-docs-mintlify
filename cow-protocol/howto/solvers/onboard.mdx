...
> ***NOTE:*** Most of this document assumes that you will be joining the solver competition under the CoW DAO bonding pool. If you were to set up your own bonding pool then most of the information in this document is either optional or irrelevant. To join the solver competition under your own bonding pool refer to [this document](https://docs.cow.fi/cow-protocol/reference/core/auctions/bonding-pools) and reach out to help set up the bonding pool and whitelist your solver's address.

![General process for joining the solver competition](/images/cow-protocol/tutorials/solvers/assets/images/solver_onboarding_process-fa171d7b521605a50e09433b7a16a114.jpg)
...
## 1. Essential information for joining the solver competition[​](#1-essential-information-for-joining-the-solver-competition "Direct link to 1. Essential information for joining the solver competition")

### Requirements for joining the CoW DAO bonding pool[​](#requirements-for-joining-the-cow-dao-bonding-pool "Direct link to Requirements for joining the CoW DAO bonding pool")
...
### Service fees[​](#service-fees "Direct link to Service fees")
...
## 2. Locally developing your solver[​](#2-locally-developing-your-solver "Direct link to 2. Locally developing your solver")
...
API specification: [https://github.com/cowprotocol/services/blob/main/crates/solvers/openapi.yml](https://github.com/cowprotocol/services/blob/main/crates/solvers/openapi.yml)

## 3. Joining the shadow (test) competition[​](#3-joining-the-shadow-test-competition "Direct link to 3. Joining the shadow (test) competition")
...
### Exposed Endpoint[​](#exposed-endpoint "Direct link to Exposed Endpoint")

The exposed endpoint provided must be of the following format: \{base\_url}/$\{envName}/$\{network} - for example: [https://api.mysolver.io/staging/mainnet](https://api.mysolver.io/staging/mainnet) envName values: shadow, staging, prod supported networks: arbitrum-one, avalanche, base, bnb, lens, linea, mainnet, polygon, xdai (gnosis) and more are coming out!
...
### Driver configuration for your solver[​](#driver-configuration-for-your-solver "Direct link to Driver configuration for your solver")

There are various configurations for your solver in the driver to help you optimize your solver:

#### - Wrapping / unwrapping native tokens[​](#--wrapping--unwrapping-native-tokens "Direct link to - Wrapping / unwrapping native tokens")
...
#### - Liquidity fetching[​](#--liquidity-fetching "Direct link to - Liquidity fetching")

It is possible to enable or disable liquidity fetching for your solver. If this is enabled, the driver will search for on-chain liquidity sources for your solver to use and send them to your solver. [Docs](https://docs.cow.fi/cow-protocol/explanation/architecture/driver#fetching-liquidity)

#### - Fairness checks for EBBO[​](#--fairness-checks-for-ebbo "Direct link to - Fairness checks for EBBO")
...
The KYC process is conducted by a third-party KYC provider. Solvers joining the CoW DAO bonding pool will typically need to provide the following documentation:

* **Certificate of Incorporation** (or equivalent formation document for your jurisdiction)
* **Shareholder register / beneficial ownership details** identifying the ultimate beneficial owners of the entity
* **Government-issued ID** (e.g. passport) for key developers and/or shareholders
* **Proof of address** for the entity (e.g. utility bill or bank statement)

The specific requirements will be explained during the onboarding call. Do not send documents before the call — the team will guide you through the submission process afterward.

## 5. Joining the staging (barn) competition[​](#5-joining-the-staging-barn-competition "Direct link to 5. Joining the staging (barn) competition")
...
## 6. Moving to production[​](#6-moving-to-production "Direct link to 6. Moving to production")
...
## 7. Weekly rewards and accounting[​](#7-weekly-rewards-and-accounting "Direct link to 7. Weekly rewards and accounting")
...
## 8. Moving to other networks[​](#8-moving-to-other-networks "Direct link to 8. Moving to other networks")
...
## 9. Joining the mainnet competition[​](#9-joining-the-mainnet-competition "Direct link to 9. Joining the mainnet competition")
...
## 10. Joining the quoting competition[​](#10-joining-the-quoting-competition "Direct link to 10. Joining the quoting competition")
...
## 11. F.A.Q.[​](#11-faq "Direct link to 11. F.A.Q.")

### Can we get access to the orderbook API?[​](#can-we-get-access-to-the-orderbook-api "Direct link to Can we get access to the orderbook API?")
...
### How do we integrate CoW AMMs?[​](#how-do-we-integrate-cow-amms "Direct link to How do we integrate CoW AMMs?")
...
### Is there a way to find out the COW reward in real time immediately after the trade?[​](#is-there-a-way-to-find-out-the-cow-reward-in-real-time-immediately-after-the-trade "Direct link to Is there a way to find out the COW reward in real time immediately after the trade?")
...
### What are the environments?[​](#what-are-the-environments "Direct link to What are the environments?")
...
### Are there any prerequisites in the shadow environment that we must meet before being allowed to join staging?[​](#are-there-any-prerequisites-in-the-shadow-environment-that-we-must-meet-before-being-allowed-to-join-staging "Direct link to Are there any prerequisites in the shadow environment that we must meet before being allowed to join staging?")
