...
### Fallback `ERC-20` Allowances[​](#fallback-erc-20-allowances "Direct link to fallback-erc-20-allowances")
...
### Balancer External Balances[​](#balancer-external-balances "Direct link to Balancer External Balances")
...
### Balancer Internal Balances[​](#balancer-internal-balances "Direct link to Balancer Internal Balances")
...
### For the Protocol[​](#for-the-protocol "Direct link to For the Protocol")

#### `transferFromAccounts`[​](#transferfromaccounts "Direct link to transferfromaccounts")
...
```
function transferFromAccounts(   GPv2Transfer.Data[] calldata transfers) external onlyCreator {   vault.transferFromAccounts(transfers, msg.sender);}
```

#### `batchSwapWithFee`[​](#batchswapwithfee "Direct link to batchswapwithfee")
...
```
function batchSwapWithFee(   IVault.SwapKind kind,   IVault.BatchSwapStep[] calldata swaps,   IERC20[] memory tokens,   IVault.FundManagement memory funds,   int256[] memory limits,   uint256 deadline,   GPv2Transfer.Data calldata feeTransfer) external onlyCreator returns (int256[] memory tokenDeltas)
```
