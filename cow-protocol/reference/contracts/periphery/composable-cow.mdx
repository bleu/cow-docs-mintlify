...
#### Programmatic order verification flow[​](#conditional-order-verification-flow "Direct link to Programmatic order verification flow")
...
A swap guard is a contract that implements the `ISwapGuard` interface, and if set by an `owner`, will be called by `ComposableCoW` prior to calling `verify` on the programmatic order.
