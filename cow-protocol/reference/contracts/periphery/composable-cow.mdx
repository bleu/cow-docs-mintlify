---
title: "ComposableCoW"
description: "ComposableCoW is a framework for smoothing the developer experience when building conditional orders on CoW Protocol. Conditional orders are a subset of  smart contract orders. It allows one to create conditional orders that:"
---
...
3. Conditional orders **SHOULD** be optimized towards *statelessness* - pass required data via `calldata`
...
#### Conditional order verification flow[​](#conditional-order-verification-flow "Direct link to Conditional order verification flow")

The following flowchart illustrates the conditional order verification flow (assuming `safe`):

#### Settlement execution path[​](#settlement-execution-path "Direct link to Settlement execution path")
...
| `typeHash`        | Not used                                                                                                                                                                                                |
| `encodeData`      | ABI-encoded [`GPv2Order.Data`](/cow-protocol/reference/contracts/core/settlement#gpv2orderdata-struct) (per [`EIP-712`](https://eips.ethereum.org/EIPS/eip-712#definition-of-encodedata)) to be settled |
| `encodeData`      | ABI-encoded [`GPv2Order.Data`](/cow-protocol/reference/contracts/core/settlement#gpv2orderdata-struct) to be settled                                                                                    |
...
### Discrete order verifiers[​](#discrete-order-verifiers "Direct link to Discrete order verifiers")
...
| `order`           | The proposed discrete order's [`GPv2Order.Data`](/cow-protocol/reference/contracts/core/settlement#gpv2orderdata-struct) struct                   |
...
### Discrete order generators[​](#discrete-order-generators "Direct link to Discrete order generators")
...
### Swap guards[​](#swap-guards "Direct link to Swap guards")
...
### `ConditionalOrderParams`[​](#conditionalorderparams "Direct link to conditionalorderparams")
...
### `PayloadStruct`[​](#payloadstruct "Direct link to payloadstruct")
...
### `singleOrders`[​](#singleorders "Direct link to singleorders")
...
### `cabinet`[​](#cabinet "Direct link to cabinet")
...
### `swapGuards`[​](#swapguards "Direct link to swapguards")
...
#### `setRoot` / `setRootWithContext`[​](#setroot--setrootwithcontext "Direct link to setroot--setrootwithcontext")
...
#### `create` / `createWithContext`[​](#create--createwithcontext "Direct link to create--createwithcontext")
...
#### `setSwapGuard`[​](#setswapguard "Direct link to setswapguard")
...
### For watch-towers[​](#for-watch-towers "Direct link to For watch-towers")

#### `getTradeableOrderWithSignature`[​](#gettradeableorderwithsignature "Direct link to gettradeableorderwithsignature")
...
3. Call `getTradeableOrder` on the handler to get the discrete order ([`GPv2Order.Data`](/cow-protocol/reference/contracts/core/settlement#gpv2orderdata-struct)).
...
### Custom error codes[​](#custom-error-codes "Direct link to Custom error codes")
...
### Watch-tower[​](#watch-tower "Direct link to Watch-tower")
