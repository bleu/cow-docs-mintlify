---
title: "TWAP"
description: "Reference for the CoW Protocol TWAP contract — data structures and parameters for time-weighted average price programmatic orders."
---

## Data Structure

```
struct Data {    IERC20 sellToken;    IERC20 buyToken;    address receiver; // address(0) if the safe    uint256 partSellAmount; // amount to sell in each part    uint256 minPartLimit; // minimum buy amount in each part (limit)    uint256 t0;    uint256 n;    uint256 t;    uint256 span;    bytes32 appData;}
```
...
<Note>
  For cancelling a TWAP order, follow the instructions at [Programmatic order cancellation](/cow-protocol/reference/contracts/periphery/composable-cow#remove).
</Note>
