...
A summary of the above, that is used to verify the accounting process each week, can be found in this Dune dashboard: [https://dune.com/cowprotocol/cow-solver-rewards](https://dune.com/cowprotocol/cow-solver-rewards).

## Auction rewards and penalties[​](#auction-rewards-and-penalties "Direct link to Auction rewards and penalties")
...
## Protocol and partner fees[​](#protocol-and-partner-fees "Direct link to Protocol and partner fees")
...
## Buffer accounting[​](#buffer-accounting "Direct link to Buffer accounting")
...
### Protocol and partner fees in the settlement contract[​](#protocol-and-partner-fees-in-the-settlement-contract "Direct link to Protocol and partner fees in the settlement contract")
...
### Network fees in the settlement contract[​](#network-fees-in-the-settlement-contract "Direct link to Network fees in the settlement contract")
...
## Payout processing and operational adjustments[​](#payout-processing-and-operational-adjustments "Direct link to Payout processing and operational adjustments")

### Service fee on COW rewards[​](#service-fee-on-cow-rewards "Direct link to Service fee on COW rewards")
...
### Minimum transfer thresholds (dust handling)[​](#minimum-transfer-thresholds-dust-handling "Direct link to Minimum transfer thresholds (dust handling)")
...
### Overdraft handling (negative net position)[​](#overdraft-handling-negative-net-position "Direct link to Overdraft handling (negative net position)")
...
Overdrafts are tracked on-chain via the overdraft management contract. When a solver’s aggregated native-token components — slippage, network fees, and COW rewards (converted to native) — sum to a negative value for a given accounting period, the deficit is recorded as an overdraft. These overdraft amounts are automatically deducted from future weekly reward payouts until the balance is cleared. Solvers also have the option to submit direct payments to the overdraft management contract to clear outstanding amounts ahead of schedule.

The overdraft management contract is deployed at `0x8fd67ea651329fd142d7cfd8e90406f133f26e8a` (same address on all networks). It exposes two relevant functions:

```solidity
// Check a solver’s outstanding overdraft balance (returns amount in wei)
function solverOverdraftBalance(address solver) external view returns (uint256);

// Pay down a solver’s overdraft — send the payment as msg.value (no amount parameter)
...
### Auction price corrections[​](#auction-price-corrections "Direct link to Auction price corrections")
