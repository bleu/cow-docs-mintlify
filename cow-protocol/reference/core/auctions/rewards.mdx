...
## Solver competition rewards (CIPs 20, 36, 38, 48, 57, 67, 74)[​](#solver-competition-rewards-cips-20-36-38-48-57-67-74 "Direct link to Solver competition rewards (CIPs 20, 36, 38, 48, 57, 67, 74)")
...
paymenti​=cap(totalScore−referenceScorei​−missingScorei​).

Here totalScore is the sum of the scores of all winning solutions in the auction and missingScorei​ is the sum of the scores of solver i's winning solutions that reverted. Finally, referenceScorei​ is the total score of a counterfactual auction in which all bids from solver i are removed from the set of bids that survive the fairness filtering.

<Note>
  The payment calculation can result in a negative figure, in which case the solver is required to pay the protocol.
</Note>

The payment is capped from above and below using the function cap(x)=max(−cl​,min(cu​,x)), where cu​ is the protocol fee (excluding partner fees) that the protocol earned from the trades in all solutions supplied by the solver in that auction and cl​ is chain-specific, determined by the following values:
...
If only one solver submits solutions, referenceScorei​ is, by definition, zero.
...
### Solver's strategy[​](#solvers-strategy "Direct link to Solver's strategy")
...
* Ethereum mainnet: min\{0.0007 ETH,6 COW},
* Gnosis Chain: min\{0.15 xDAI,6 COW},
* Arbitrum: min\{0.00024 ETH,6 COW},
* Base Chain: min\{0.00024 ETH,6 COW},
* Avalanche-C Chain: min\{0.006 AVAX,6 COW},
* Polygon Chain: min\{0.6 POL,6 COW}
* Lens Chain: min\{0.15 GHO,6 COW}
* BNB Chain: min\{0.001 BNB,6 COW}
* Linea: min\{0.00003 ETH,6 COW}.
* Plasma: min\{0.6 XPL,6 COW}.
* Ink: min\{0.00003 ETH,6 COW}.
