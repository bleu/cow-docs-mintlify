...
sidebarTitle: "The solving problem"
description: "Mathematical overview of CoW Protocol solving — order selection, routing, and bidding as a two-step optimization problem."
---
...
We also assume that $0 \in S$ that is, when submitting an order a user accepts that the order may not be filled. Also, to each order $S$ we define *surplus function* $U_S: S \to \mathbb{R}$, measuring "how good" a trade is from the point of view of the user who submitted order $S$. By definition $U_S(0) = 0$.
...
A *buy order* is specified by a maximum buy amount $X > 0$ and a limit price $\pi$ corresponding to the worst-case exchange rate the user is willing to settle for. With $x$ denoting the buy amount and $y$ denoting the sell amount of the order, fill-or-kill limit buy orders have the form

$$
S = \left\{ \begin{bmatrix} x \\ -y \end{bmatrix} \text{ s.t. } y \leq x \cdot \pi \text{ and } x \in \{0, X\} \right\}
$$
...
The main difference is that the limit price $\pi$ corresponds to the price at which a zero-fee traditional AMM would trade. For example, in the case of a simple, constant product CoW AMM pool with reserves $X$ and $Y$, we have
...
Each user order may have an associated fee paid to the protocol. At a high level, these fees can be represented by a function that, for a given order $S$ maps all possible trades to a non-negative vector of tokens, that is $f_S: S \to \mathbb{R}^k_+$ with $f_S(0) = 0$.
...
* $o$ is a sell order or a CoW AMM order: $\text{score}(o) = (U(o) + f(o)) \cdot p(b)$, where $p(b)$ is an externally provided price of the buy token relative to a numéraire.
* $o$ is a buy order: $\text{score}(o) = (U(o) + f(o)) \cdot p(b) \cdot \pi$, where $p(b)$ is an externally provided price of the buy token relative to a numéraire and $\pi$ is the limit price of the order.
