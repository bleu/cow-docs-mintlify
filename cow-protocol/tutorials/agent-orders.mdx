...
- **Tokens to trade** — the sell token must already be in the wallet
...
<Warning>
Never hardcode private keys. Use environment variables (`os.environ["PRIVATE_KEY"]`) or a secrets manager. The examples below use a placeholder for clarity.
</Warning>
...
- **`sellAmount`** = `quote.sellAmount + quote.feeAmount` (the full amount before network cost deduction)
- **`buyAmount`** = the slippage-adjusted minimum from Step 2
- **`feeAmount`** = `0` (fees are handled by the settlement contract)
