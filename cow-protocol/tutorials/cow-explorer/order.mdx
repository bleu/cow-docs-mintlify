---
title: "By order"
description: "A user may want to see the details of a specific order that they have placed. They can search for this order by its unique identifier, the . This identifier is returned when an order is placed and can be used to query the details of the order."
---
...
## Fill-or-kill[​](#fill-or-kill "Direct link to Fill-or-kill")
...
![Order overview](/images/cow-protocol/tutorials/cow-explorer/assets/images/order_overview-1a72b779d95e0ee3c0f6bce551de54c9.png)

<Accordion title="Understanding order details">
  | **Field**            | **Description**                                                                                                                                                                                                 |
  | -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
## Partially fillable[​](#partially-fillable "Direct link to Partially fillable")

A more complex example may be a user who has placed a *partially fillable* order. This means that the user has expressed a swap intent for a certain amount of tokens, but the order may be filled in multiple batches. This can only be used with [Limit orders](/cow-protocol/tutorials/cow-swap/limit).

![Partially fillable order overview](/images/cow-protocol/tutorials/cow-explorer/assets/images/order_partial_overview-ceb851992c2b686a760527a39e9bc2ae.png)
...
![Partially fillable order fills](/images/cow-protocol/tutorials/cow-explorer/assets/images/order_partial_fills-72389e4647e0cf1e7a590a18ddfa2a6f.png)
...
* **Open** (pending)**:** State for a standing order. Orders in this state are being considered by the solvers. It is the entry state for all orders[1](#user-content-fn-presign), the default from the moment the order is placed. From here the order can transition to all other states.
...
1. Except for pre-signed orders, where the first state is **Pre-signing**. [↩](#user-content-fnref-presign)
