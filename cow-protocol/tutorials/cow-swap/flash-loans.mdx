---
title: "Flash loans"
description: "We’ll walk through a concrete example of a debt repayment using flash loans on Sepolia."
---
...
## Aave repay pre-hook[​](#aave-repay-pre-hook "Direct link to Aave repay pre-hook")
...
```
const repayTxData = encodeFunctionData({    abi, // ABI definition for the repay function    functionName: 'repay',    args: [      '0xaA8E23Fb1079EA71e0a56F48a2aA51851D8433D0', // asset: USDT      '5000000000', // amount: 5,000 USDT (USDT has 6 decimals)      2, // interestRateMode: 2 for variable      '0x35eD9A9D1122A1544e031Cc92fCC7eA599e28D9C', // onBehalfOf: Safe address    ]  });
```
...
To enable this, the pre-hook defines a transaction that will be sent to the Safe wallet. The Safe then executes the actual repay call to the Aave pool. The transaction can be built using the [Safe Protocol Kit](https://docs.safe.global/sdk/protocol-kit) as shown below:

```
  const repaySafeTxData: SafeTransactionDataPartial = {    to: '0x6Ae43d3271ff6888e7Fc43Fd7321a503ff738951', // Aave pool address in Sepolia    value: '0',    data: repayTxData, // created previously    operation: OperationType.Call,    nonce, // current Safe nonce + 1  };  const safeTransaction = await safe.createTransaction({ transactions: [repaySafeTxData] });  const signedSafeTransaction = await safe.signTransaction(safeTransaction, SigningMethod.ETH_SIGN);  const encodedSafeTransaction = await safe.getEncodedTransaction(signedSafeTransaction);
```
...
## Aave withdraw pre-hook[​](#aave-withdraw-pre-hook "Direct link to Aave withdraw pre-hook")
...
```
  const txData = encodeFunctionData({    abi, // ABI definition for the withdraw function    functionName: 'withdraw',    args: [      '0x94a9d9ac8a22534e3faca9f4e7f2e2cf85d5e4c8', // asset: USDC      '400000000000', // amount: 400,000 USDC (USDC has 6 decimals)      '0x35eD9A9D1122A1544e031Cc92fCC7eA599e28D9C', // to: Safe address    ]  });
```
...
```
  const safeTxData: SafeTransactionDataPartial = {    to: '0x6Ae43d3271ff6888e7Fc43Fd7321a503ff738951', // Aave pool address in Sepolia    value: '0',    data: txData, // created previously    operation: OperationType.Call,    nonce, // current Safe nonce + 2  }  const safeTransaction = await safe.createTransaction({ transactions: [safeTxData] })  const signedSafeTransaction = await safe.signTransaction(safeTransaction, SigningMethod.ETH_SIGN);  const encodedSafeTransaction = await safe.getEncodedTransaction(signedSafeTransaction);  return encodedSafeTransaction;
```
...
## Building and publishing the order[​](#building-and-publishing-the-order "Direct link to Building and publishing the order")

With both the `repay` and `withdraw` pre-hooks in place, we can now build the metadata of our order:

```
  const appData = {    metadata: {      flashloan: {        lender: config.AAVE_POOL_ADDRESS,        token: USDT_ADDRESS,        amount: "5000000000" // 5,000 USDT      },      hooks: {        pre: [          {            target: config.SAFE_ADDRESS,            value: "0",            callData: repayTx, // repay pre-hook data            gasLimit: "1000000"          },          {            target: config.SAFE_ADDRESS,            value: "0",            callData: withdrawTx, // withdraw pre-hook data            gasLimit: "1000000"          }        ],        "post": []      },      signer: config.SAFE_ADDRESS,    },  };
```
...
## Setting the pre-signature of the order[​](#setting-the-pre-signature-of-the-order "Direct link to Setting the pre-signature of the order")
...
## Order execution[​](#order-execution "Direct link to Order execution")
...
This error means the settlement contract's `isValidSignature` check failed during simulation. Common causes:

| Cause | Fix |
|---|---|
| Safe not configured as the order owner | Set `from` to the Safe address in the quote request |
| `signingScheme` mismatch | Use `SigningScheme.PRESIGN` for Safe-based flash loan orders |
| `setPreSignature` not called | Submit the PreSign transaction before the order can be filled |
| Wrong chain domain separator | Verify `chainId` matches the target network |

### Order stuck / not filling

Flash loan orders are complex and some solvers may skip them:

- **Verify the order appears** in [CoW Explorer](https://explorer.cow.fi)
- **Check the order status** via `GET /api/v1/orders/{uid}` — if `presignaturePending`, the PreSign tx hasn't confirmed
- **Increase validity** — flash loan orders may need more batches to find a solver willing to execute them
- **Check token approvals** — the Safe must approve VaultRelayer for the sell token (collateral)

### Adapting the example to other networks

The example uses Sepolia. When moving to mainnet or other chains:

- Update the Aave pool address for the target network
- Verify the [flash loan router is deployed](/flash-loan-router/operations/networks) on your target chain
- Update token addresses (USDC, USDT addresses differ per chain)
- The settlement contract and VaultRelayer addresses are the same on all chains
