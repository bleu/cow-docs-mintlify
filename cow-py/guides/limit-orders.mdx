...
Limit orders fill when the market price reaches your specified rate or better. Unlike [`swap_tokens`](/cow-py/guides/swapping-tokens) which executes immediately at the current market price, limit orders may take hours, days, or never fill if the market doesn't reach your target.
...
<Tip>
  Fetch a market quote with `api.post_quote()` to see the current price before placing your limit order. This helps you set a realistic limit relative to where the market is right now. See [Using a Quote as Price Reference](#using-a-quote-as-price-reference) above.
</Tip>
...
For the full set of order management operations, see [Managing Orders](/cow-py/guides/managing-orders).
...
|---|---|---|
| `InsufficientAllowance` error | Vault Relayer not approved | Approve the sell token first (see [Token Approval Flow](/cow-py/guides/swapping-tokens#token-approval-flow)) |
