...
description: "Advanced programmatic order types including TWAP orders and custom programmatic mechanisms"
---
...
<ResponseField name="orderBookApi" type="OrderBookApi" required>
OrderBook API instance
</ResponseField>

<ResponseField name="offChainInput" type="string">
...
<ResponseField name="blockInfo" type="BlockInfo">
...
Poll result indicating order status
  <Expandable>
    <ResponseField name="result" type="PollResultCode">
    `SUCCESS` | `TRY_NEXT_BLOCK` | `TRY_ON_BLOCK` | `TRY_AT_EPOCH` | `DONT_TRY_AGAIN` | `UNEXPECTED_ERROR`
    </ResponseField>
    <ResponseField name="order" type="GPv2Order">
...
<ResponseField name="receiver" type="string" required>
Address to receive the bought tokens
</ResponseField>

<ResponseField name="sellAmount" type="bigint" required>
Total amount to sell across all parts (in token atoms)
</ResponseField>

<ResponseField name="buyAmount" type="bigint" required>
Minimum total amount to buy across all parts (in token atoms)
</ResponseField>

<ResponseField name="numberOfParts" type="bigint" required>
Number of parts to split the order into (must be > 1)
</ResponseField>

<ResponseField name="timeBetweenParts" type="bigint" required>
Duration between each part in seconds (max 1 year)
</ResponseField>

<ResponseField name="startTime" type="StartTime" required>
When the TWAP should start
  <Expandable>
    <ResponseField name="startType" type="StartTimeValue" required>
    `AT_MINING_TIME` | `AT_EPOCH`
    </ResponseField>
    <ResponseField name="epoch" type="bigint">
    Unix timestamp for start (required if startType is AT_EPOCH)
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="durationOfPart" type="DurationOfPart" required>
How long each part remains valid
  <Expandable>
    <ResponseField name="durationType" type="DurationType" required>
    `AUTO` | `LIMIT_DURATION`
    </ResponseField>
...
<ResponseField name="appData" type="string" required>
...
<ResponseField name="orders" type="Record<string, ConditionalOrder>">
...
<ResponseField name="proofLocation" type="ProofLocation">
...
<ResponseField name="proofStruct" type="ComposableCoW.ProofStruct">
  <Expandable>
    <ResponseField name="location" type="ProofLocation">
