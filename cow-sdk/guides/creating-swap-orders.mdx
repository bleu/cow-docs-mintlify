...
description: "How to create market orders using the CoW Protocol SDK's postSwapOrder method"
---
...
- **Token approval**: The sell token must be approved to the CoW Protocol Vault Relayer contract before the order can settle. Without approval, solvers cannot pull tokens from your wallet. See [Token Approvals](/cow-sdk/guides/token-approvals) for full details.
...
<Note>
  Off-chain cancellation asks solvers to stop filling the order. It takes effect almost immediately but is not enforced on-chain. For guaranteed cancellation of smart contract orders, see [Order Management](/cow-sdk/guides/order-management).
</Note>
...
- Learn about [Token Approvals](/cow-sdk/guides/token-approvals) before trading
- Explore [Limit Orders](/cow-sdk/guides/creating-limit-orders) for price-specific trades
