...
graph TB
    ...
    D -->|Check Conditions| E[Programmatic order SDK]
...
| `CONDITIONAL_ORDER_REGISTRY_{chainId}` | Map of all active programmatic orders by owner | `Map<Owner, Set<ConditionalOrder>>` |
...
if (filterPolicy) {
  ...
  if (filterResult === policy.FilterAction.DROP) {
    log.info("Not adding the programmatic order. Reason: AcceptPolicy: DROP");
    return false;
  }
}
```
