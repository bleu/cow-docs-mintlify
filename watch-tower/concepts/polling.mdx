---
title: "Order Polling"
description: "Learn how the Watch Tower polls programmatic orders and submits them to the CoW Protocol OrderBook"
---

# Order Polling

Order polling serves as the fundamental mechanism enabling the Watch Tower to assess programmatic orders and determine their readiness for CoW Protocol OrderBook submission. This continuous process evaluates every active programmatic order with each new blockchain block.

## Polling overview

The Watch Tower evaluates programmatic orders by invoking their handler contracts to verify whether order conditions have been satisfied. Upon successful condition validation, the handler delivers a discrete order suitable for OrderBook posting.
...
For each block, the Watch Tower processes all registered owners and their associated programmatic orders:

```typescript
for (const [owner, conditionalOrders] of ownerOrders.entries()) {
  for (const conditionalOrder of conditionalOrders) {
    // Poll each programmatic order
  }
}
```
...
The Watch Tower engages the programmatic order handler to assess condition satisfaction:
...
<Info>
If a handler remains unrecognized by the SDK, the Watch Tower defaults to [legacy polling](#legacy-polling) using direct contract calls to `getTradeableOrderWithSignature`. This means **custom handlers work automatically** — the Watch Tower will discover and poll any handler deployed via ComposableCoW, regardless of whether it's in the SDK registry.
</Info>

To build your own custom handler, see:
* [Custom Orders Guide](/composable-cow/advanced/custom-orders) — implementing `IConditionalOrder` and `IConditionalOrderGenerator`
* [IConditionalOrder Interface](/composable-cow/api/interfaces/iconditional-order) — API reference for the handler interface
* [Creating Orders](/composable-cow/guides/creating-orders) — registering orders with the ComposableCoW contract
...
* `PollTryAtEpoch(uint256 epoch, uint256 blockTimestamp)`: Try again at specific time
* `PollTryAtBlock(uint256 blockNumber)`: Try again at specific block
* `PollTryNextBlock`: Try again on the next block
* `OrderNotValid(string reason)`: Order is not currently valid
* `PollNever(string reason)`: Stop trying

<Warning>
**Unknown reverts permanently stop polling (Issue #325)**

If `getTradeableOrderWithSignature` reverts with bytes that do not decode to one of the known custom errors listed above, the Watch Tower classifies the result as `UNEXPECTED_ERROR` and **permanently stops polling that order**. This applies to any revert that produces unrecognized error data, including:

- Arithmetic overflows or underflows (Solidity 0.8+ panic codes)
- Out-of-gas in sub-calls
- Generic `revert()` or `require()` without a matching custom error
- Reverts from external contract calls that propagate unknown error selectors

The order creator receives **no notification** when this happens. The order silently disappears from the polling cycle.

**All revert paths in your handler must use one of the recognized custom errors.** If your handler logic can fail for any reason, wrap it in a try/catch and revert with `OrderNotValid(reason)` as a fallback. See [Custom Orders — Error Communication](/composable-cow/advanced/custom-orders#error-communication) for the full list of recognized errors.
</Warning>
