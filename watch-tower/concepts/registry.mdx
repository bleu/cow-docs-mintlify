...
description: "Learn how the Watch Tower stores and manages programmatic order state"
---

# Registry and State Management

The Registry serves as the core state management system for the Watch Tower, maintaining a persistent record of all active programmatic orders, their owners, and associated metadata across restarts.
...
The primary data structure maps owners to their programmatic orders:
...
Each programmatic order stores its most recent poll result:
...
Stores the serialized registry of all programmatic orders:
...
When a new programmatic order is detected:
...
* `activeOrdersTotal`: Current number of active programmatic orders
