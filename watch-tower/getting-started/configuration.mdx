...
Configure the Watch Tower to monitor blockchain networks and manage programmatic orders.
...
**deploymentBlock** (integer, required)
...
**filterPolicy** (object, required)
Defines filtering rules for programmatic orders. See [Filter policy](#filter-policy) below.
...
The filter policy controls which programmatic orders are processed by the Watch Tower.
...
**conditionalOrderIds** (object)
Map of programmatic order IDs to filter actions. Use this to block or allow specific order IDs.
...
<Warning>
Programmatic order types may consume considerable RPC calls. Monitor your RPC provider's rate limits and usage.
</Warning>
...
- Learn about [deployment options](/watch-tower/deployment/docker)
