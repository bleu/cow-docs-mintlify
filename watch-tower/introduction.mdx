...
```mermaid
graph LR
    A[User Creates<br/>Programmatic Order] --> B[ComposableCoW<br/>Contract]
...
See [Deployed Contracts](https://github.com/cowprotocol/composable-cow#deployed-contracts) for block numbers on each chain.

## Next steps

* Learn about the [Architecture](/watch-tower/architecture) and how the watch tower processes events
* Review configuration options in the example `config.json.example`
* [Build a custom programmatic order handler](/composable-cow/advanced/custom-orders) — the Watch Tower will automatically discover and poll any handler deployed via `ComposableCoW`
